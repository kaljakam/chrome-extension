...
document.addEventListener("DOMContentLoaded",
    function (event) {
        // При загрузке страницы внедряем скрипт
...
// Слушаем сообщения со страницы из внедренного скрипта
window.addEventListener("message",
    function (event) {
        if (event.source == window && event.data.To == "Server") {
            ...
            Port.onMessage.addListener(
                function (message, Port) {
                    window.postMessage({ Data: message, To: "Client" }, "*");
                }
            );
        };
    }
...
chrome.runtime.onMessage.addListener(
    function (request, sender, sendResponse) {
        if (request.name == "kkmserver.addin.EventBarcode") {
            window.postMessage({ name: "kkmserver.addin.EventBarcode", BarCode: request.message.Event.Data, NumDevice: request.message.Event.Source }, "*");
        };
    });
